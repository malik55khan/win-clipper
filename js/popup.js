...
var popup = {
		...
		chrome.tabs.query({ active: true, currentWindow: true }, function (tab) { // null defaults to current window
			...
			if (isRobotsMetaTagMatch || isRobotsNoIndexMatch) {
				var lassoURL = protocol + urlPrefix + 'lasso/view-item?url=' +
					encodeURIComponent(url) + '&title='+title+'&desc=&login=true&isOpenedFromExtension=true';
			} else {
				for (var i = 0; i < metas.length; i++) {
					if (metas[i].getAttribute('name') && metas[i].getAttribute('name')
	...
	addNote: function () {
		chrome.tabs.query({ active: true, currentWindow: true }, function (tab) { // null defaults to current window
			...
			if (isRobotsMetaTagMatch || isRobotsNoIndexMatch) {
				var lassoURL = protocol + urlPrefix + 'lasso/view-item?url=' +
					encodeURIComponent(url) + '&title='+title+'&desc=&login=true&app=1&isOpenedFromExtension=true';
			} else {
				for (var i = 0; i < metas.length; i++) {
					if (metas[i].getAttribute('name') && metas[i].getAttribute('name')
						.toLowerCase() == 'description') {
		...
		chrome.tabs.query({ active: true, currentWindow: true }, tabs => {
			if (tabs.length) {
				chrome.tabs.sendMessage(tabs[0].id, data, function (x) {
					if (x == undefined) alert("Please refresh page and try again.")
					window.close();
				});
			}
		})
