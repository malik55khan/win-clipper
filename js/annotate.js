...
var actualWidthToConsiderForCanvas,actualHeightToConsiderForCanvas;
                    
if (!localStorage['lastTool']) localStorage['lastTool']='crop';

function editor_obj()
	{
 	var theColor='#000000',lineWidth=2;
	...
	var tool={current:'rectangle',status:'ready'};
...
//		c.fillStyle=currentLevel.color;
	...
	tool.undo.click=function ()
		...
		if(currentLevel.type=='delete')	{
			...
			if (currentIndex<0)
				{
				clip.rx1=clip.ry1=0;
				clip.ry2=$('#screenshot').height();
				clip.rx2=$('#screenshot').width();
				}
			else
				{
				currentLevel=levels[currentIndex];
	...
	this.createLastCanvas=function(todo,moreParams)
		...
		if (imageChanged!=localStorage['pngjpg'])
			...
			ctx.drawImage(   $('#screenshot')[0],clip.rx1,clip.ry1,clip.rx2-clip.rx1, clip.ry2-clip.ry1 ,0,0,clip.rx2-clip.rx1, clip.ry2-clip.ry1 )

            for (var i = 0; i <= currentIndex; i++) {
                if (cancel) { noMoreWait(); return; }
                var thisLevel = levels[i];
                var actualx = thisLevel.x;
                var actualy = thisLevel.y;
                var actualStartx = thisLevel.start.x;
                var actualStarty = thisLevel.start.y;
                var actualEndx = thisLevel.end.x;
                var actualEndy = thisLevel.end.y;

                if (thisLevel.type == 'bubble' || (thisLevel.text && thisLevel.text != '')) {
                    thisLevel.x = this.getActualCalculatedPointValue(thisLevel.x, 'width');
                    thisLevel.y = this.getActualCalculatedPointValue(thisLevel.y, 'height');
                    thisLevel.start.x = this.getActualCalculatedPointValue(thisLevel.start.x, 'width');
                    thisLevel.start.y = this.getActualCalculatedPointValue(thisLevel.start.y, 'height');
                    
                } else {
                    thisLevel.start.x = this.getActualCalculatedPointValue(thisLevel.start.x, 'width');
                    thisLevel.start.y = this.getActualCalculatedPointValue(thisLevel.start.y, 'height');
                    thisLevel.end.x = this.getActualCalculatedPointValue(thisLevel.end.x, 'width');
                    thisLevel.end.y = this.getActualCalculatedPointValue(thisLevel.end.y, 'height');
                }
                ...
                if (thisLevel.type != 'crop' && thisLevel.type != 'delete' && !thisLevel.hide){
                    ...
                    if (thisLevel.type == 'bubble') {
                        ctx.font = "35px Arial";
                        placeTextInCanvas(ctx, thisLevel.text, thisLevel.x, thisLevel.y+10, 
                       30, parseInt(thisLevel.width),thisLevel.start.x-10,thisLevel.start.y-10);
                    } else {
                        tool[thisLevel.type].draw({ canvas: done, ctx: ctx, data: thisLevel });
                    }
                }
                thisLevel.x=actualx;
                thisLevel.y=actualy;
                thisLevel.start.x=actualStartx;
                thisLevel.start.y=actualStarty;
                thisLevel.end.x=actualEndx;
                thisLevel.end.y=actualEndy;
...
//				return;
        ...
        this.getActualCalculatedPointValue =function(coordinateValue,type){
            var calculatedVal=coordinateValue;
            if(type=='width'){
                calculatedVal = coordinateValue*actualWidthToConsiderForCanvas/$("#screenshot").width();
            }else if(type=='height'){
                calculatedVal = coordinateValue*actualHeightToConsiderForCanvas/$("#screenshot").height();
            }
	...
	this.init=function()
		...
		// 	hideSpeed: 300,
		// 	buttonSize: 26,
		// 	initColor:localStorage['color'] || '#000000',
		// 	onSelect:function (a) {
		...
		var myInt;
		var	$this;
		$('#screenshot').on('mousemove',function (e) {
...
//					,zIndex:this.style.zIndex-1
		...
		canvas=$('canvas')[1];
					...
					// 	// })
					// 	editor.reloadCanvas();
                    var tmpImageToGrabWidthHeight = new Image(); 
                    tmpImageToGrabWidthHeight.onload = function(){
                    actualWidthToConsiderForCanvas=tmpImageToGrabWidthHeight.width;
                    actualHeightToConsiderForCanvas=tmpImageToGrabWidthHeight.height;
                    };
                    tmpImageToGrabWidthHeight.src = screenshot.canvas.toDataURL(); 
					img.onload=function (){
						canvas.width=actualWidthToConsiderForCanvas
						canvas.height=actualHeightToConsiderForCanvas
						firstImage=this
...
tool.bubble.begin = function (e) {
    ...
    e.x -= baseRectDim / 2
    e.y -= baseRectDim / 2
    currentLevel.type = 'bubble';
    currentLevel.start = { x: e.x + 15, y: e.y + 15 };
    currentLevel.end = { x: e.x + baseRectDim, y: e.y + baseRectDim };
    tool.bubble.move({ x: e.x + baseRectDim, y: e.y + baseRectDim });

};
tool.bubble.move = function (e) {
...
drawTextArea = function () {
    ...
    e = textAreaPosition;
    //	tool.status='started';
    startX = textAreaPosition.x + 16;
    startY = textAreaPosition.y + 16;
    ...
    $('#text_helper')
        .css({
            'z-index': 10000,
            left: currentLevel.start.x, //+$canvasData.offset().left,
            top: currentLevel.start.y, //+$canvasData.offset().top,
            height: currentLevel.end.y - currentLevel.start.y//clip.ry2-startY>70?70:clip.ry2-startY//clip.ry2-startY
            , width: currentLevel.end.x - currentLevel.start.x//clip.rx2-startX>200?200:clip.rx2-startX//clip.rx2-startX
        })
    b = ['left', 'top', 'z-index', 'width', 'height', 'position', 'font-weight', 'width', 'fontWeight'];
    var ta = $('<textarea></textarea>');
    ta.css('z-index', 10000);
...
finishTextArea = function () {
    a = {
        text: $('textarea').val(),
        font: '14px' + ' ' + $('textarea').css('font-family'),
   ...
   $i.remove();;
   c.clearRect(0, 0, event.clientX, event.clientY);
//  $(canvas).remove();
    $('textarea').remove();
    //  newLevel();
	// 	createCanvas();
    ...
    ctx.font = "20px Arial";
    debugger;
    placeTextInCanvas(ctx, currentLevel.text, 10, 10, parseInt('20px'), parseInt(currentLevel.width),0,0)
    updateImgFromCanvas();
    
}

placeTextInCanvas = function (ctx, text, x, y, lineHeight, fitWidth,rectangleStartX,rectangleStartY) {
    text = text.replace(/(\r\n|\n\r|\r|\n)/g, "\n");
    sections = text.split("\n");
    var allTextToFill = [];
    ...
    if(allTextToFill.length==0 || (allTextToFill.length==1 && allTextToFill[0].text=='') ){
        return;
    }
    if(!rectangleStartX){
        rectangleStartX=0;
    ...
    ctx.fillRect(rectangleStartX, rectangleStartY, maxWidth + 20, maxHeight);
    ctx.fillStyle = "#000000";
    ctx.strokeRect(rectangleStartX, rectangleStartY, maxWidth + 20, maxHeight);

    $.each(allTextToFill, function (i, textInfo) {
...
////    Free
	...
	tool.arrow.draw=function(inX)
		...
		function drawArrow(x1,y1,x2,y2,shadow)
			...
			inX.ctx.lineTo(x2 + DeltaY * dy / L - DeltaX * dx1 / L,
		               y2 - DeltaX * dy / L - DeltaY * dx1 / L -1);

		// //	inX.ctx.lineTo(x2 - DeltaY * dy / L  - DeltaX * dx1 / L, y1);

        //Code-ZB Below code added by  for to correct tringle pointer.. but need more evaluation and testing
       /** console.log("x1:"+x1+" "+"x2:"+x2+" y1:"+y1+" "+"y2:"+y2+" "+DeltaX+" "+" " +DeltaY)
            var xToConsider=x1>x2?x2-2:x2+2;
            var yToConsider=y2;
            if(x2>x1){
                xToConsider=x2+2;
                yToConsider=y2-1;
            ...
            if(DeltaX<0 && DeltaY<0){
             ...
             inX.ctx.lineTo(xToConsider, yToConsider);*/
              inX.ctx.lineTo(x2, y2);
		...
		$('#toolbar .button').each(function()	{
			...
			if ($(this).attr('status')=="on"){
				$(this).attr({'src':'images/drawing/' + $(this).attr('tag')+ ($(this).attr("tag")==tool.current? 'On' : 'Off') + '.png'})
						.css({'cursor':'pointer'});
				$(this)[$(this).attr("tag") ==tool.current ? 'addClass':'removeClass']('active');
				}
			else
...
//************Binds****************
  ...
  $("#divCanvasData").bind('mousedown', function (e) {
    if(textAreaOn){
        textAreaOn=false;
        e.stopImmediatePropagation();
        finishTextArea();
    ...
    if (e.button == 0) {
      ...
      if (e.delegateTarget.id == "divCanvasData") {
        return false;
      }
    }
  });
	$("#divCanvasData").bind('mousemove',function (e)
		{
        dime=pos(e);
...
//	$(document.body).bind('keydown','alt+z',function ()
		...
		$('#toolbar .button').click(function ()
            ...
            $('.choose-color').click(function (){
                theColor=$(this).data('colorcode');
                $("#colorPickerButton").css("background-color",theColor);
                $("#colorPickerExpansionDiv").hide();
                $("#colorPickerButton").removeClass('active');
        ...
        if (version && version >= 76) {
            navigator.permissions.query({ name: 'clipboard-write' }).then(result => {
                if (result.state === 'granted') {
                    var item = new ClipboardItem({ 'image/png': scope.image_blob() });
...
if (location.hash == "#paste") {
  ...
  $(function () {
    ...
    $("html").pasteImageReader(function (e, t, n) {
      ...
      r.src = e.dataURL, canvas = $("canvas")[1], img = $("#screenshot")[0];
      img.onload = function () {
