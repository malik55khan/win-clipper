...
$(function () {
    chrome.storage.local.get((storage) => {
        let screenshot = storage.screenshot;
        console.log("storagestorage", screenshot)
        ...
        function showLassoAddItemInFrame() {
            console.log("showLassoAddItemInFrame")
            $("#loader").show();
            resourceRefId = new Date().getTime();
            document.getElementById("resourceRefId").value = "" + resourceRefId + "";
            $('#MainPopupIframeAddItem').attr("src", serverUrlBasePath + "/go/lasso/add-item2?title=" + '' + "&desc=" + '' +"&actionType=3&app=1&url=" + encodeURIComponent(screenshot.screenShotParams.url) + "&extURL=" + encodeURIComponent(window.location.href) + "&resourceRefId=" + resourceRefId);
        }
